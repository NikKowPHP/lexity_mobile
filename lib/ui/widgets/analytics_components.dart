  ...
  Widget build(BuildContext context) {
    return GlassCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Review Forecast", style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              const Text("Weekly Goal", style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
              Text(
          ...
          LinearProgressIndicator(
            ...
            color: percent >= 1.0 ? Colors.greenAccent : LiquidTheme.primaryAccent,
            minHeight: 8,
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              _ActivityIcon(icon: Icons.check_circle_outline, count: progress.breakdown.modules, label: "Modules"),
              _ActivityIcon(icon: Icons.edit_note, count: progress.breakdown.journals, label: "Journals"),
              _ActivityIcon(icon: Icons.psychology, count: progress.breakdown.sessions, label: "Sessions"),
            ],
          )
        ],
      ),
...
class _ActivityIcon extends StatelessWidget {
  ...
  const _ActivityIcon({required this.icon, required this.count, required this.label});
...
class ProficiencyLineChart extends StatelessWidget {
  ...
  const ProficiencyLineChart({super.key, required this.history, required this.prediction});
  ...
  Widget build(BuildContext context) {
    ...
    if (history.isNotEmpty && prediction.isNotEmpty) {
       predictionSpots.insert(0, FlSpot((history.length - 1).toDouble(), history.last.score));
    }
    ...
    return GlassCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Proficiency Over Time", style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
          ...
          AspectRatio(
            ...
            child: LineChart(
              LineChartData(
                ...
                titlesData: const FlTitlesData(show: false), // Simplified for mobile
                ...
                lineBarsData: [
                  LineChartBarData(
                    ...
                    belowBarData: BarAreaData(show: true, color: LiquidTheme.primaryAccent.withValues(alpha: 0.1)),
                  ),
  ...
  Widget build(BuildContext context) {
     ...
     return GlassCard(
       child: Column(
         crossAxisAlignment: CrossAxisAlignment.start,
         children: [
           const Text("Activity (Last 14 Days)", style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
           const SizedBox(height: 12),
           Row(
             mainAxisAlignment: MainAxisAlignment.spaceBetween,
             children: last14Days.map((date) {
               final dateStr = DateFormat('yyyy-MM-dd').format(date);
               final point = data.firstWhere((p) => p.date == dateStr, orElse: () => ActivityHeatmapPoint(date: dateStr, totalSeconds: 0));
               final minutes = point.totalSeconds / 60;
               
               Color color = Colors.white10;
               if (minutes > 0) color = LiquidTheme.primaryAccent.withValues(alpha: 0.3);
               if (minutes > 15) color = LiquidTheme.primaryAccent.withValues(alpha: 0.6);
